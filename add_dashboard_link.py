#!/usr/bin/env python3
"""
Script to add Dashboard link to Home workspace
Run this script using: bench --site <site_name> execute new_theme.add_dashboard_link.add_dashboard_link
"""
	...
	setInterval(function() {
		...
		if (iframe && iframe.contentWindow) {
			try {
				...
				frappe.db.commit()
				print("✓ Dashboard Custom HTML Block created")
			...
			print("✓ Dashboard added to Home workspace content")
			return {"status": "success", "message": "Dashboard added to Home workspace content"}
		else:
			print("✓ Dashboard already exists in Home workspace")
